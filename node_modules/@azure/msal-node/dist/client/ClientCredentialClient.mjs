...
'use strict';
import { Constants, TokenCacheContext, ScopeSet, TimeUtils, ResponseHandler, createClientAuthError, ClientAuthErrorCodes, UrlString, RequestParameterBuilder, getClientAssertion, StringUtils, UrlUtils } from '@azure/msal-common/node';
...
class ClientCredentialClient extends BaseClient {
    ...
    async getCachedAuthenticationResult(request, config, cryptoUtils, authority, cacheManager, serverTelemetryManager) {
        ...
        let lastCacheOutcome = Constants.CacheOutcome.NOT_APPLICABLE;
        ...
        if (!cachedAccessToken) {
            serverTelemetryManager?.setCacheOutcome(Constants.CacheOutcome.NO_CACHED_ACCESS_TOKEN);
        ...
        if (TimeUtils.isTokenExpired(cachedAccessToken.expiresOn, clientConfiguration.systemOptions?.tokenRenewalOffsetSeconds ||
            ...
            serverTelemetryManager?.setCacheOutcome(Constants.CacheOutcome.CACHED_ACCESS_TOKEN_EXPIRED);
        ...
        if (cachedAccessToken.refreshOn &&
            ...
            serverTelemetryManager?.setCacheOutcome(Constants.CacheOutcome.PROACTIVELY_REFRESHED);
        }
        return [
    ...
    async createTokenRequestBody(request) {
        ...
        RequestParameterBuilder.addGrantType(parameters, Constants.GrantType.CLIENT_CREDENTIALS_GRANT);
